...
class MemberGroupPrivDataModel extends xModel{
	...
	private function repairArrParentid($parentid,$arrparentid){
		...
		foreach($data_top as $k => $v ){
			unset($data_top[$k]);
			$this->clearWhere()->where(array('priv_id'=>$v['priv_id']))->update(['arrparentid'=>$arrparentid]);
			$sub_tree_data = $this->clearWhere()->where(array('parentid'=>$v['priv_id']))->getAll();
			foreach($sub_tree_data as $k2 => $v2 ){
				unset($sub_tree_data[$k2]);
				$this->repairArrParentid($v['priv_id'],$v['arrparentid'].','.$v['priv_id']);
			}
		}
	}
	
	private function repairArrChildid(){
		$this->update(['arrchildid='=>'priv_id']);
		$data_top = $this->getAll();
		
		foreach($data_top as $k => $v ){
			unset($data_top[$k]);
			$this->clearWhere()->where('priv_id in ('.$v['arrparentid'].')')
				->update(['arrchildid='=>'CONCAT(arrchildid,",'.$v['priv_id'].'")']);
		}
	}
