...
class TreeCommon {
	...
	//试题列表专用
	static public function createTreeChapterOptionToQuestionList($parme1, $parme2){
		$parentid = xInput::request('parentid',0);
		$subjectid = xInput::request('subjectid',0);
		$select = $style = 'style="display:none"';
		if ($parentid == $parme1 ['treeid'])$select = 'selected';
		if ($subjectid == $parme1 ['subjectid'])$style = 'style="display:block"';
		$tree = '<option subjectid="'.$parme1 ['subjectid'].'" '.$style.' value="' . $parme1 ['treeid'] . '" ' . $select . '>' . $parme2 . $parme1 ['chaptername'] . '</option>';
		return $tree;
	}
	
	
	//创建以table方式显示的树形菜单
	static public function createTreeChapterTableRule($parme1, $parme2){
		$temp = explode(',',$parme1 ['arrparentid']);
		$style = 'style="display:none"';
		if(count($temp)<=2) $style = 'style="display:table-row"';
		...
		$tree = '<tr parentid="' . $parme1 ['parentid'] . '" treeid="' . $parme1 ['treeid'] . '" arrchildid="' . $parme1 ['arrchildid'] . '" '.$style.' >
		<td align="center">' . $parme1 ['treeid'] . '</td>
		<td class="break-all">' . $parme2 . cutStr($parme1 ['chaptername'],15) . '</td>';
			
		foreach($GLOBALS['question_type'] as $k => $v ){
			$number = 0;
			$model_value = 2;
			$model_checked_1 = $model_checked_2 = '';
			if(isset($GLOBALS['rule_cfg_injection'][$v['typeid'].'-'.$parme1['treeid']])){
				$number = $GLOBALS['rule_cfg_injection'][$v['typeid'].'-'.$parme1['treeid']]['number'];
				$model_value = $GLOBALS['rule_cfg_injection'][$v['typeid'].'-'.$parme1['treeid']]['model'];
			}
			if($model_value==1){
				$model_checked_1 = 'checked';
			}else{
				$model_checked_2 = 'checked';
			}
			$tree .= '
			<td align="center">
				<label><input name="rule[cfg]['.$parme1['treeid'].']['.$v['typeid'].'][number]" data-type="'.$v['modelclass'].'" value="'.$number.'" type="text" class="w-20"></label>
				<label><input name="rule[cfg]['.$parme1['treeid'].']['.$v['typeid'].'][model]" data-type="'.$v['modelclass'].'" data-class="modelselect" type="radio" value="1" '.$model_checked_1.'>独占</label>
				<label><input name="rule[cfg]['.$parme1['treeid'].']['.$v['typeid'].'][model]" data-type="'.$v['modelclass'].'" data-class="modelselect" type="radio" value="2" '.$model_checked_2.'>兼顾</label>
				<label><input name="rule[cfg]['.$parme1['treeid'].']['.$v['typeid'].'][typeid]" value="'.$v['typeid'].'" type="hidden"></label>
			</td>';
		}
		$tree .= '</tr>';
