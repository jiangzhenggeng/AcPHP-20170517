...
class StrongAction extends ApiCommon  {

    public function getSubjectQuestionTypeList(){
        $subjectModel = M('subject');
        $subjectData_temp = $subjectModel->order('listorder desc,subjectid desc')->getAll();
		$subjectData = $subjectIds = [];
		foreach($subjectData_temp as $k => $v ){
			$subjectData[$v['subjectid']] = $v;
			$subjectIds[] = $v['subjectid'];
		}
		unset($subjectData_temp);
		
        $subject_type = M('subject_question_type')->where([
			'subjectid'=>[$subjectIds,'in']
		])->getAll();
		
		$type_data_temp = M('question_type')->getAll();
		$type_data = [];
		foreach($type_data_temp as $k => $v ){
			$type_data[$v['typeid']] = $v;
		}
		
		$typeids = [];
		foreach($subject_type as $k => $v ){
			$subjectData[$v['subjectid']]['question_type'][] = array_merge($v,$type_data[$v['typeid']]);
		}
		$subjectData_temp = [];
		foreach($subjectData as $k => $v ){
			$subjectData_temp[] = $v;
		}
        xOut::json(outSuccess($subjectData_temp));
    }

    public function getChapterList(){

        $subjectid = intval(xInput::request('subjectid'));
        $subjectid >0 || xOut::json(outError('参数错误[1]'));
        $typeid = intval(xInput::request('typeid'));
        $typeid >0 || xOut::json(outError('参数错误[2]'));
        ...
        $number = M('question')->where([
            'subjectid'=>$subjectid,
            'typeid'=>$typeid,
        ])->count();

        xOut::json(outSuccess(array('count'=>$number,'chapterList'=>$subjectData_temp)));
    ...
    public function getQuestionList(){
        ...
        $where = array(
            'typeid'=>$typeid,
            'treeid'=>[$arrchildidData['arrchildid'],'in'],
            'ischeck'=>1
        );
    ...
    public function saveRecord(){
        ...
        foreach ($question_type as $k => $v ){
            ...
            // 1考试2练习3强化4模拟
            $result = $typeClassObject->score($answer_questionid[$v['typeid']],$memberid,3);
    ...
    public function strongList(){
        ...
        $Model = M('member_practice')->where(array(
            'memberid'=>$memberid,
            'type'=>3
        ))->order('practiceid desc')->limit('0,150');
