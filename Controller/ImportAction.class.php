...
class ImportAction extends Action{	
	...
	public function major(){
		...
		foreach($professional as $k => $v){
            $count = $majorModel->clearWhere()->where('majorid='.$v['professionalid'])->count();
            if( $count ){
                continue;
            }
            $majorModel->clearWhere()->insert(array(
	...
	public function subject(){
		...
		foreach($subject as $k => $v){
            ...
            $count = M('subject')->clearWhere()->where('subjectid='.$v['subjectid'])->count();
            if( $count ){
                continue;
            }
	...
	public function subject_tree(){
		...
		foreach($category as $k => $v){
            ...
            $count = M('subject_tree')->clearWhere()->where('treeid='.$v['catid'])->count();
            if( $count ){
                continue;
            }
    ...
    public function questionType(){
        ...
        $subject_ids = [];
        foreach ($subject as $k => $v ){
            $count = M('subject_question_type')->where('subjectid='.$v['subjectid'])->count();
            if($count<=0){
                $subject_ids[] = $v['subjectid'];
            }
        }
        ...
        foreach($subject_ids as $k => $v){
            ...
            foreach($question_type as $k2 => $v2 ) {
                M('subject_question_type')->insert(array(
                    'subjectid' => $v,
                    'typeid' => $v2['typeid']
                ));
            }
